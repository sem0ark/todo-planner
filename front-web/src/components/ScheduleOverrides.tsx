...
export default function ScheduleOverrides() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await updateScheduleOverride(token, date, { day_template_id: id });
  ...
  const handleRemove = async (overrideDate: string) => {
    ...
    try {
      await updateScheduleOverride(token, overrideDate, { day_template_id: null });
  ...
  return (
    <div className="w-full max-w-3xl">
      <h2 className="text-2xl font-semibold text-snow mb-6">Schedule Overrides</h2>
      ...
      <div className="mb-6 p-4 bg-slate-blue/10 border border-slate-grey rounded-lg">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-cloud mb-2">Date</label>
            <input
              type="date"
              value={date}
          ...
          <div>
            <label className="block text-sm font-medium text-cloud mb-2">Template</label>
            <select
              value={templateId}
      ...
      <div>
        <h3 className="text-lg font-semibold text-snow mb-4">Future Overrides</h3>
        {futureOverrides.length === 0 ? (
          <p className="text-center text-cloud py-8">No future overrides scheduled.</p>
        ) : (
          <div className="space-y-2">
            {futureOverrides.map((override) => (
              ...
              >
                <span className="w-32 text-snow font-mono">{override.calendar_date}</span>
                <span className="flex-1 text-snow">{getTemplateName(override.day_template_id)}</span>
                <button

...
func (r *CategoryRepository) FindByUser(ctx context.Context, userID int) ([]BlockCategory, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&cat.ID, &cat.UserID, &cat.Name, &cat.Color, &cat.CreatedAt, &cat.UpdatedAt); err != nil {
			return nil, err
		}
...
func (r *CategoryRepository) FindByID(ctx context.Context, id, userID int) (*BlockCategory, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT id, user_id, name, color, is_deleted, created_at, updated_at
		FROM block_categories
		WHERE id = $1 AND user_id = $2
	`, id, userID).Scan(&cat.ID, &cat.UserID, &cat.Name, &cat.Color, &cat.IsDeleted, &cat.CreatedAt, &cat.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &cat, nil
...
func (r *CategoryRepository) Create(ctx context.Context, input CategoryInput, userID int) (*BlockCategory, error) {
	...
	err := r.db.QueryRow(ctx, `
		INSERT INTO block_categories (user_id, name, color, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, user_id, name, color, created_at, updated_at
	`, userID, input.Name, input.Color, now, now).Scan(
		&cat.ID,
		&cat.UserID,
		&cat.Name,
		&cat.Color,
...
func (r *CategoryRepository) Update(ctx context.Context, id int, input CategoryInput, userID int) (*BlockCategory, error) {
	...
	err := r.db.QueryRow(ctx, `
		...
		RETURNING id, user_id, name, color, created_at, updated_at
	`, input.Name, input.Color, now, id, userID).Scan(
		&cat.ID,
		&cat.UserID,
		&cat.Name,
		&cat.Color,

...
func main() {
	...
	http.HandleFunc("/account", loggingMiddleware(corsMiddleware(api.protectedHandler(api.accountRouter))))
	http.HandleFunc("/settings", loggingMiddleware(corsMiddleware(api.protectedHandler(api.settingsRouter))))
	http.HandleFunc("/devices", loggingMiddleware(corsMiddleware(api.protectedHandler(api.devicesRouter))))
	http.HandleFunc("/categories", loggingMiddleware(corsMiddleware(api.protectedHandler(api.categoriesRouter))))
	http.HandleFunc("/categories/", loggingMiddleware(corsMiddleware(api.protectedHandler(api.categoriesRouter))))
	http.HandleFunc("/schedule", loggingMiddleware(corsMiddleware(api.protectedHandler(api.scheduleRouter))))
	http.HandleFunc("/schedule/", loggingMiddleware(corsMiddleware(api.protectedHandler(api.scheduleRouter))))
	http.HandleFunc("/template-groups", loggingMiddleware(corsMiddleware(api.protectedHandler(api.templateGroupsRouter))))
	http.HandleFunc("/template-groups/", loggingMiddleware(corsMiddleware(api.protectedHandler(api.templateGroupsRouter))))
	http.HandleFunc("/templates", loggingMiddleware(corsMiddleware(api.protectedHandler(api.templatesRouter))))
	http.HandleFunc("/templates/", loggingMiddleware(corsMiddleware(api.protectedHandler(api.templatesRouter))))
